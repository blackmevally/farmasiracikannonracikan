...
document.addEventListener("DOMContentLoaded", () => {
    const streamURL = "http://192.168.9.136/hls/stream.m3u8"; // Sesuaikan jika pakai port 8080
    ...
    fetch(streamURL, { method: "HEAD" })
        .then(res => {
            if (res.ok) {
                ...
                console.warn("Stream tidak aktif:", streamURL);
            }
        })
...
async function playVoiceWithEffect(template, nomor, loket, boxId, color) {
    ttsQueue = ttsQueue.then(async () => {
        ...
        // 🔹 Perbaikan teks duplikat "loket loket"
        ...
        sse.addEventListener("update", (e) => {
            ...
            setTimeout(() => {
                if (isRacikan) {
                    ...
                    document.getElementById("loketRacikan").textContent = "Menuju " + loket.replace(/^loket\s+/i, "");
                    ...
                    document.getElementById("loketObat").textContent = "Menuju " + loket.replace(/^loket\s+/i, "");
